...
export const CHECKOFF_LOCATION = 'CHECKOFF_LOCATION'
export const CHECKON_LOCATION = 'CHECKON_LOCATION'
...
export const checkoffLocation = (country) => {
  return async (dispatch) => {
    let thecountry = await axios.put(`http://localhost:8000/checkofflocation/${country.id}`, country)
    dispatch({
      type: CHECKOFF_LOCATION,
      payload: thecountry.data
    })
  }
}

export const checkonLocation = (country) => {
  return async (dispatch) => {
    let thecountry = await axios.put(`http://localhost:8000/checkonlocation/${country.id}`, country)
    dispatch({
      type: CHECKON_LOCATION,
      payload: thecountry.data
    })
  }
}
